...
def main(region):
    ...
    try:
                                ...
                                for key in ['IpPermissions', 'IpPermissionsEgress']:
                                    ...
                                    for rule in sg[key]:
                                        if rule['IpProtocol'] == '-1':
                                            ...
                                            rules_str = "\tProtocol: {}\n".format(rule['IpProtocol'])
                                        # Get ports
                                        if 'FromPort' in rule:
                                            rules_str += "\t  PortRange: {}-{}\n".format(rule['FromPort'], rule['ToPort'])

                                        # Get CIDRs
                                        cidrs = [cidr['CidrIp'] for cidr in rule['IpRanges']]
                                        if len(cidrs) > 0:
                                            rules_str += "\t  CIDRs: {}\n".format(','.join(cidrs))
                                        ...
                                        if 'UserIdGroupPairs' in rule:
                                            if len(rule['UserIdGroupPairs']) > 0:
                                                ...
                                                for source_sg in rule['UserIdGroupPairs']:
                                                    ...
                                                    dash = '- '
                                                    for field in fields:
                                                        rules_str += "\t  {}{}: {}\n".format(dash, field, source_sg[field])
                                                        dash = '  '
        ...
        unused_sgs_report = []
        unused_sgs_report.append(['VpcId', 'GroupName', 'GroupId', 'Description', 'Rules'])
